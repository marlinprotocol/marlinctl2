...
type runner02resource struct {
	...
	ProxyProgram, ProxyUser, ProxyRunDir, ProxyExecutablePath, TcpAddress, VsockAddress string 
}

const (
	runner02proxyName               = "ip-to-vsock"
	runner02proxyProgramName        = "tcptovsockproxy"
	runner02defaultUser              = "root"
	runner02supervisorConfFiles      = "/etc/supervisor/conf.d"
	runner02proxySupervisorConfFile = "proxy_tcptovsock"
	runner02logRootDir               = "/var/log/supervisor"
	runner02oldLogRootDir            = "/var/log/old_logs"
	runner02projectName              = "proxy_tcptovsock"
)
...
func (r *linux_amd64_supervisor_runner02) Create(runtimeArgs map[string]string) error {
	...
	substitutions := runner02resource {
		...
		runner02proxyProgramName + "_" + r.InstanceId, currentUser.Username, currentUser.HomeDir, r.Storage + "/" + r.Version + "/" + runner02proxyName, "127.0.0.1:8000", "99:9000",
	}
	...
	gt := template.Must(template.New("proxy-template").Parse(util.TrimSpacesEveryLine(`
		[program:{{.ProxyProgram}}]
		process_name={{.ProxyProgram}}
		user={{.ProxyUser}}
		directory={{.ProxyRunDir}}
		command={{.ProxyExecutablePath}} --ip-addr {{.TcpAddress}} --vsock-addr {{.VsockAddress}}
		priority=100
		numprocs=1
		stdout_logfile=/var/log/supervisor/{{.ProxyProgram}}-stdout.log
		stderr_logfile=/var/log/supervisor/{{.ProxyProgram}}-stderr.log
	`)))
	gFile, err := os.Create(runner02supervisorConfFiles + "/" + runner02proxySupervisorConfFile + r.InstanceId + ".conf")
	if err != nil {
		return err
	}
	defer gFile.Close()
...
func (r *linux_amd64_supervisor_runner02) PostRun() error {
	var proxyConfig = runner02supervisorConfFiles + "/" + runner02proxySupervisorConfFile + r.InstanceId + ".conf"
