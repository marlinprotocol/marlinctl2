...
func init() {
	app, err := appcommands.GetNewApp("gateway_cosmos", projectRunners.GetRunnerInstance,
		appcommands.CommandDetails{Use: "create", DescShort: "Create gateway for cosmos blockchain", DescLong: "Create gateway for cosmos blockchain", AdditionalPreRunTest: keystore.KeystoreCheck},
		...
		appcommands.CommandDetails{Use: "recreate", DescShort: "Recreate end to end gateway (cosmos) instances", DescLong: "Recreate end to end gateway (cosmos) instances", AdditionalPreRunTest: keystore.KeystoreCheck},
