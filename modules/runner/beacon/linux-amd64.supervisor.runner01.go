...
func (r *linux_amd64_supervisor_runner01) Create(runtimeArgs map[string]string) error {
	...
	substitutions := resource{
		...
		beaconProgramName + r.InstanceId, currentUser.Username, currentUser.HomeDir, r.Storage + "/" + r.Version + "/" + beaconName, "127.0.0.1:8002", "127.0.0.1:8003", "", "", "",
	}
	...
	gt := template.Must(template.New("beacon-template").Parse(util.TrimSpacesEveryLine(`
		...
		command={{.BeaconExecutablePath}} {{if .DiscoveryAddr}} --discovery_addr "{{.DiscoveryAddr}}"{{end}}{{if .HeartbeatAddr}} --heartbeat_addr "{{.HeartbeatAddr}}"{{end}}{{if .BootstrapAddr}} --beacon_addr "{{.BootstrapAddr}}" --keystore_path "{{.KeystorePath}}" --keystore_path_pass "{{.KeystorePassPath}}" {{end}}
		priority=100
		numprocs=1
		numprocs_start=1
		autostart=true
