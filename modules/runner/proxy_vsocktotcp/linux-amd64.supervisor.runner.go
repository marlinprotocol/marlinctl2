...
const (
	runner02proxyName               = "vsock-to-ip"
	runner02proxyProgramName        = "vsocktotcpproxy"
	runner02defaultUser              = "root"
	runner02supervisorConfFiles      = "/etc/supervisor/conf.d"
	runner02proxySupervisorConfFile = "proxy_vsocktotcp"
	runner02logRootDir               = "/var/log/supervisor"
	runner02oldLogRootDir            = "/var/log/old_logs"
	runner02projectName              = "proxy_vsocktotcp"
)
...
func (r *linux_amd64_supervisor_runner02) Create(runtimeArgs map[string]string) error {
	...
	gt := template.Must(template.New("proxy-template").Parse(util.TrimSpacesEveryLine(`
		...
		command={{.ProxyExecutablePath}} --vsock-addr {{.VsockAddress}} --ip-addr {{.TcpAddress}}
		priority=100
		numprocs=1
